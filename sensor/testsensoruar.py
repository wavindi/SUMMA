...
# Flag to suppress status messages during calibration
        ...
        try:
            ...
            if count > 0:
                ...
                for line in lines:
                    ...
                    # ✅ FIX 1: Only print startup messages, suppress frame count messages
                    elif line and not calibration_mode:
                        if (line.startswith("CONFIG") or 
                            line.startswith("READY") or 
                            line.startswith("ERROR")):
                            print(f"  [{sensor_name}] {line}")
                        # Suppress "PICO: Frame X | Errors: Y" messages completely
                        # (they happen every 100 frames and clutter output)
            else:
                time.sleep(0.001)
...
def calibrate_pico_baseline(sensor_data, sensor_lock, name):
    ...
    calibration_mode = True  # ✅ Suppress status messages during calibration
    
    samples = {i: [] for i in range(16)}
    collected = 0
    timeout = time.time() + 15  # ✅ FIX 2: Increase timeout from 10s to 15s
    
    # ✅ FIX 3: Wait for initial data availability
    wait_start = time.time()
    while time.time() - wait_start < 5:  # Wait up to 5 seconds for first data
        with sensor_lock:
    ...
    with sensor_lock:
        if not sensor_data['data_ready']:
            print(f"⚠️ WARNING: {name} not sending data yet - calibration may fail")
            print(f"   Check wiring: Pico TX (GP0) → Pi GPIO")
    ...
    calibration_mode = False  # ✅ Re-enable status messages after calibration
    
    # ✅ FIX 4: Show warning if calibration failed
    if collected == 0:
        print(f"❌ {name} calibration FAILED (0 samples collected)")
        print(f"   Troubleshooting:")
        print(f"   1. Check Pico is powered and sensor LED is on")
        print(f"   2. Verify TX wire: Pico GP0 → Pi GPIO")
        print(f"   3. Check common ground connection")
        print(f"   4. Try connecting Pico to laptop USB and check Serial Monitor")
    else:
...
def main():
    ...
    with pico1_sensor_lock:
        ...
        pico1_frame_count = pico1_sensor_data['frame_count']
    with pico2_sensor_lock:
        pico2_ready = pico2_sensor_data['data_ready']
        pico2_frame_count = pico2_sensor_data['frame_count']
    
    if not pico1_ready:
        print(f"⚠️ WARNING: No data from Pico 1 yet (frame count: {pico1_frame_count})")
        print("   Check: Pico 1 GP0 → Pi GPIO 23, common ground, Pico powered")
    else:
        print(f"✅ Pico 1 streaming data (frame count: {pico1_frame_count})")
    
    if not pico2_ready:
        print(f"⚠️ WARNING: No data from Pico 2 yet (frame count: {pico2_frame_count})")
        print("   Check: Pico 2 GP0 → Pi GPIO 24, common ground, Pico powered")
    else:
        print(f"✅ Pico 2 streaming data (frame count: {pico2_frame_count})")
