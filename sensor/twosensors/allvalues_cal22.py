#!/usr/bin/env python3
"""
VL53L5CX Dual Sensor - WITH OUTLIER THRESHOLD
Blocks 2000mm spikes, allows smooth transitions
"""
...
# Outlier rejection
OUTLIER_THRESHOLD = 50  # Blocks spikes, allows 40mm→0mm transitions
...
def check_outlier(current_value, last_value, threshold):
    ...
    if last_value is None:
        return False  # First value is never an outlier
    
    difference = abs(current_value - last_value)
    if difference > threshold:
        return True  # It's an outlier
    return False
...
def main():
    ...
    print("="*100)
    print("VL53L5CX DUAL SENSOR - CALIBRATED WITH OUTLIER REJECTION")
    print("="*100)
    print(f"Median: {MEDIAN_WINDOW}, Moving Avg: {MOVING_AVG_WINDOW}, Outlier Threshold: {OUTLIER_THRESHOLD}mm\n")
    ...
    print("\n" + "="*100)
    print("LIVE MEASUREMENT PHASE - WITH OUTLIER REJECTION")
    print("="*100 + "\n")
    ...
    with open(filename, "w") as log_file:
        ...
        log_file.write(f"Outlier Threshold: {OUTLIER_THRESHOLD}mm (rejects spikes, allows smooth transitions)\n")
        ...
        log_file.write("\n" + "="*100 + "\n")
        log_file.write("LIVE MEASUREMENTS (corrected with outlier rejection):\n")
        ...
        try:
            while True:
                ...
                # Read Sensor 1 - CORRECTED WITH OUTLIER REJECTION
                if sensor1.data_ready():
                    try:
                        data1 = sensor1.get_data()
                        for zone in range(16):
                            try:
                                ...
                                if check_outlier(corrected, last_valid_s1[zone], OUTLIER_THRESHOLD):
                                    # Reject outlier, use last valid value
                ...
                # Read Sensor 2 - CORRECTED WITH OUTLIER REJECTION
                if sensor2.data_ready():
                    try:
                        data2 = sensor2.get_data()
                        for zone in range(16):
