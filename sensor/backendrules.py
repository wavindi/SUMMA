...
Using padel-specific rules:
- Normal games: first to 4 points with 2-point lead
- Normal tie break: to 7 with 2-point lead (at 6–6 in games, sets 0–0 or 0–1)
- Super tie break: to 10 with 2-point lead (decider at 1–1 in sets)
Points are stored as 0,1,2,... and UI should display 1,2,3,...
"""
...
game_state = {
    ...
    # Points in current game or tie break (internal 0,1,2,...; UI shows +1)
    'point_1': 0,
    'point_2': 0,

    # Legacy display fields (not used for logic, can be used by UI if needed)
    'score_1': 0,
    'score_2': 0,
    ...
    # History
    'set_history': [],     # e.g. ["6-3", "7-5"]
    'match_history': [],   # detailed actions
...
# ============================================================================
# SET / MATCH WIN LOGIC WITH PADEL RULES
# ============================================================================

def check_set_winner():
    """
    Normal set logic:
    - A team wins a set when it wins 6+ games with a 2-game lead.
    - At 6–6 games:
      * If match sets are 0–0 or 0–1: start normal tie break (mode='tiebreak').
      * If match sets are 1–1: use super tie break instead of full third set (mode='supertiebreak').
    This function handles only normal-set completion; tie-break completion is handled
    inside process_add_point when in tiebreak/supertiebreak mode.
    """
    global game_state
    ...
    # Normal set win (6 or more with 2-game lead)
    if g1 >= 6 and (g1 - g2) >= 2:
        set_before = (s1, s2)
        game_state['set_1'] += 1
...
def check_match_winner():
    """Match is best of 3 sets: first to 2 sets wins."""
    global game_state
...
# ============================================================================
# PADDEL POINT LOGIC: NORMAL, TIE BREAK, SUPER TIE BREAK
# ============================================================================

def _increment_points(team):
    """Increment internal points for given team (0,1,2,...) and update display scores."""
    if team == 'black':
        game_state['point_1'] += 1
    else:
        game_state['point_2'] += 1

    # Legacy score fields mirror point values (UI can map to 1,2,3,...)
...
def _handle_normal_game_win(team):
    """Called when someone wins a normal game."""
    if team == 'black':
        game_state['game_1'] += 1
    else:
        game_state['game_2'] += 1
...
def _handle_tiebreak_win(team):
    """Tie break: single set, normal match score (0–0 or 0–1 sets)."""
    g1 = game_state['game_1']
    g2 = game_state['game_2']

    # In tie break, winner gets this set; game score is stored as 7–6 (or 6–7).
    if team == 'black':
        game_state['set_1'] += 1
        game_state['set_history'].append(f"{g1 + 1}-{g2}")  # 7-6 or similar
    else:
        game_state['set_2'] += 1
        game_state['set_history'].append(f"{g1}-{g2 + 1}")  # 6-7 or similar
...
def _handle_supertiebreak_win(team):
    """Super tie break: decider when sets are 1–1; winner gets match."""
    # Treat super tie break as final set: set_1 or set_2 increments to 2
    if team == 'black':
        game_state['set_1'] += 1
    else:
        game_state['set_2'] += 1

    game_state['set_history'].append(f"STB-{game_state['point_1']}-{game_state['point_2']}")
    _reset_points()
    game_state['mode'] = 'normal'
    check_match_winner()
...
def process_add_point(team):
    """
    Process add point with padel rules:
    - Normal games: first to 4 points with 2-point lead.
    - Normal tie break: to 7 with 2-point lead.
    - Super tie break: to 10 with 2-point lead.
    """
    global game_state
    ...
    # NORMAL TIE BREAK
        ...
        if team == 'black':
            if p1 >= 7 and (p1 - p2) >= 2:
                ...
                action_type = 'game'  # treat as game/set event in history
        else:
            if p2 >= 7 and (p2 - p1) >= 2:
    ...
    # Record history if match not already finished
    if not game_state['match_won']:
        if action_type in ['point', 'game']:
    ...
    # Side switch only on normal game wins (not on tie-break points)
...
def process_subtract_point(team):
    """
    Subtract point in the simplest possible way:
    - If in normal mode, just decrement point counters (cannot undo games/sets safely).
    - Does not touch mode/tiebreak logic to keep it predictable.
    """
    global game_state
