...
def send_action_http(team, action, detection_time):
    """
    OPTIMIZATION: Ultra-fast HTTP with backend confirmation
    - Uses persistent HTTP session (keep-alive)
    - Low timeout values
    - Triggers relay ONLY after backend confirms
    - Returns immediately to not block loop
    """
    if action == 'add':
        url = ADD_POINT_URL
    ...
    def _send_and_trigger():
        """Inner function: Send HTTP and trigger relay on success"""
        try:
            # Timestamp with milliseconds for accuracy
            start_time = time.time()
            timestamp = time.strftime('%H:%M:%S.%f')[:-3]
            
            print(f"📤 [{timestamp}] Sending: {action_text} for {team.upper()} ({detection_time:.2f}s)")
            
            # OPTIMIZATION: Fast HTTP with separate connection and read timeouts
            response = http_session.post(
                url, 
                json=payload, 
                timeout=(HTTP_CONNECTION_TIMEOUT, HTTP_TIMEOUT)  # (connect, read)
            )
            ...
            if response.status_code == 200 and response.json().get('success'):
                ...
                # Check for side switch notification
                if response_data.get('side_switch'):
                    print(f"🔄 [{timestamp_confirm}] SIDE SWITCH REQUIRED!")
                    print(f"   Total games played: {response_data['side_switch']['total_games']}")
                    print(f"   Current score: {response_data['side_switch']['game_score']}")
                    print(f"   🏃 Players should change court sides now!")
            ...
            print(f"⚠️  [{timestamp}] Backend timeout (>{HTTP_TIMEOUT*1000:.0f}ms)")
            return False
        except Exception as e:
            print(f"❌ [{timestamp}] HTTP Error: {e}")
            return False
    
    # Run in separate thread so main loop isn't blocked
...
def process_sensor(sensor, baseline, median_windows, movavg_windows, last_valid):
    ...
    try:
        ...
        for i in range(16):
            ...
            # Auto-reset filter on large changes
...
def process_single_sensor(state, detected, current_time, timestamp, team_info):
    ...
    if detected:
        ...
        # Check for timeout (hand held too long)
        ...
        # Hand is NOT present
        if state['active']:
            # Hand was just removed - SEND REQUEST IMMEDIATELY
            ...
            if action:
                # Send HTTP immediately (runs in background thread)
                # Relay will trigger when backend confirms
...
def main():
    print("="*70)
    print("🏓 Padel Scoreboard - OPTIMIZED with Side Switching")
    print("="*70)
    print("⚡ OPTIMIZATIONS:")
    ...
    print(f"   • Side switch: After odd-numbered games (1, 3, 5, 7...)")
    print("="*70)
    ...
    print("\n🚀 System Ready - Ultra-Low Latency Mode with Side Switching!")
    ...
    try:
        while True:
            ...
            # Process local sensor (Black team)
            ...
            # Process remote sensor (Yellow team)
            ...
            # OPTIMIZATION: 5ms sleep = 200 updates/second for instant hand removal detection
